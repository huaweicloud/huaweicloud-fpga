...
- constraints  
  该目录主要用于存放Example2的自定义约束信息，来源于`usr_prj_cfg` 中的 USR_CONSTRAINTS。
...
- usr_prj_cfg  
  该文件主要用于配置example2工程的自定义信息，详细使用说明请见[usr_prj_cfg使用说明](#sec-3-1)。
...
- usr_prj_cfg 文件在`$WORK_DIR/hardware/vivado_design/example/example2/prj/`目录中（后续所有操作均在此目录执行）。

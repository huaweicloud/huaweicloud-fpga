...
- constraints  
  该目录主要用于存放Example3的自定义约束信息，来源于`usr_prj_cfg` 中的 USR_CONSTRAINTS。
...
- usr_prj_cfg  
  该文件主要用于配置example3工程的自定义信息，详细使用说明请见[usr_prj_cfg使用说明](#sec-3-1)。
...
- usr_prj_cfg 文件在`$WORK_DIR/hardware/vivado_design/example/example3/prj/`目录中（后续所有操作均在此目录执行）。
...
单步执行pr校验打印提示出现`“PR_VERIFY: check points /huaweicloud-fpga/.../example3/prj/build/checkpoints/to_facs/usr_prjxx_routed.dcp and /huaweicloud-fpga/.../lib/checkpoints/SH_UL_BB_routed.dcp are compatible”`表示PR校验成功；
...
该命令主要完成dcp文件的打包上传等步骤，执行该文件即可实现将dcp文件上传至OBS桶，并获取注册ID。

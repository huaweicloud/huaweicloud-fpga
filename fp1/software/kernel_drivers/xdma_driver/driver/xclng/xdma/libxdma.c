...
static void engine_status_dump(struct xdma_engine *engine)
{
	...
	if ((v & XDMA_STAT_READ_ERR)) {
		len += sprintf(buf + len, "READ_ERR:");
		if ((v & XDMA_STAT_R_UNSUPP_REQ))
			len += sprintf(buf + len, "UNSUPP_REQ ");
		if ((v & XDMA_STAT_R_COMPL_ABORT))
			len += sprintf(buf + len, "COMPL_ABORT ");
		if ((v & XDMA_STAT_R_PARITY_ERR))
			len += sprintf(buf + len, "PARITY ");
		if ((v & XDMA_STAT_R_HEADER_EP))
			len += sprintf(buf + len, "HEADER_EP ");
		if ((v & XDMA_STAT_R_UNEXP_COMPL))
			len += sprintf(buf + len, "UNEXP_COMPL ");
		buf[len - 1] = ',';
	}

 	if ((v & XDMA_STAT_DESC_ERR)) {
	...
	if ((engine->dir == DMA_FROM_DEVICE) && (v & XDMA_STAT_WRITE_ERR)) {
		len += sprintf(buf + len, "WRITE_ERR:");
		if ((v & XDMA_STAT_W_DECODE_ERR))
			len += sprintf(buf + len, "DECODE_ERR ");
		if ((v & XDMA_STAT_W_SLAVE_ERR))
		...
		struct xdma_transfer *transfer, u32 desc_completed)
{
	...
	if (printk_ratelimit()) {
		pr_info("%s, s 0x%x, aborted xsfer 0x%p, cmpl %d/%d\n",
			...
			int offset, enum dma_data_direction dir, int channel)
{
	...
	if (!engine->desc)
		pr_warn("device %s, engine %s pre-alloc desc failed.\n",
			dev_name(&xdev->pdev->dev), engine->name);
	else
		dbg_init("device %s, engine %s pre-alloc desc 0x%p,0x%llx.\n",
			dev_name(&xdev->pdev->dev), engine->name,
...
static void transfer_destroy(struct xdma_dev *xdev,
			struct xdma_transfer *transfer, bool force)
{
	/* free descriptors */
	xdma_desc_done(transfer->desc_virt);
			...
			u64 ep_addr, struct scatterlist **sgl_p, int nents)
{
	...
	if (!engine->desc) {
		engine->desc = dma_alloc_coherent(&xdev->pdev->dev,
			...
			&engine->desc_bus, GFP_KERNEL);
		if (!engine->desc) {
			pr_err("device %s, engine %s pre-alloc desc failed.\n",
				dev_name(&xdev->pdev->dev), engine->name);
			goto err_out;
		}
		dbg_init("device %s, engine %s pre-alloc desc 0x%p,0x%llx.\n",
	...
	transfer_desc_init(transfer, nents);
...
int xdma_xfer_submit(void *dev_hndl, int channel, bool write, u64 ep_addr,
			struct sg_table *sgt, bool dma_mapped, int timeout_ms)
{
	...
	/* check the direction */
	if (engine->dir != dir) {
		pr_info("channel 0x%p, %s, %d, W %d, 0x%x/0x%x mismatch.\n",
	...
	while (nents) {
		...
		if (rv < 0) {
			...
			transfer_destroy(xdev, transfer, 1);
			return rv;
		}
		...
		case TRANSFER_STATE_COMPLETED:
			...
			dbg_tfr("transfer %p, %u completed, done %lu.\n",
				transfer, transfer->xfer_len, done);
		...
		transfer_destroy(xdev, transfer, 0);
	} /* while (sg) */

			...
			int offset, enum dma_data_direction dir, int channel)
{
	int rv;
	u32 val;
	char nameTmp[19] = {0};		/* name of this engine */
	...
	sprintf(nameTmp, "%d-%s%d-%s", xdev->idx,
		(dir == DMA_TO_DEVICE) ? "H2C" : "C2H", channel,
		engine->streaming ? "ST" : "MM");
    strncpy(engine->name, nameTmp, sizeof(engine->name) - 1);
						...
						u64 ep_addr)
{
	...
	for (i = 0;  (i < max) && (sg != 0); i++, sg = sg_next(sg)) {
...
//pr_info("ep 0x%llx, desc %u+%u.\n", ep_addr, max, extra);
	...
	for (i = 0, sg = sgt->sgl;  (i < sgt->nents) && (sg != 0); i++, sg = sg_next(sg)) {
				...
				struct pci_dev *pdev)
{
	...
	for (i = 0; (i < npages) && (sg != 0); i++, sg = sg_next(sg)) {

...
static int probe_feature_rom(struct drm_xocl_dev *xdev)
{
	...
	// Magic number check
	if (val != 0x786e6c78) {
      if (xdev->ddev->pdev->vendor == 0x19e5 && (xdev->ddev->pdev->device == 0xd512 || xdev->ddev->pdev->device == 0xf000)) {
        printk(KERN_INFO "XOCL: Found HW VU9P Device without featureROM\n");
        //This is HW device. Fill the FeatureROM struct. Right now it doesn't have FeatureROM
        ...
        strncpy(xdev->header.FPGAPartName, "HW VU9P", 8);
        memset(xdev->header.VBNVName, 0, sizeof(xdev->header.VBNVName));
        strncpy(xdev->header.VBNVName, "huawei_vu9p_dynamic_fp1_5_0", 35);
        ...
        printk(KERN_INFO "XOCL: Enabling HW dynamic 5.0 DSA\n");
...
static int xocl_info_udev_ioctl(struct drm_device *dev,
			void *data,
			struct drm_file *filp)
{
	struct drm_xocl_info_udev*obj = data;
	struct drm_xocl_dev *xdev = dev->dev_private;
	struct pci_dev *pdev = xdev->ddev->pdev;
	printk(KERN_INFO "%s %s UDEV INFO IOCTL \n", DRV_NAME, __FUNCTION__);
	...
	obj->subsystem_device = pdev->subsystem_device;
      ...
      memcpy_fromio(&xdev->header, xdev->user_bar + XOCL_FEATURE_ROM, sizeof(struct FeatureRomHeader));
	obj->ddr_channel_count = xdev->header.DDRChannelCount;
      obj->ddr_channel_size= xdev->header.DDRChannelSize;
      memcpy(obj->vbnv,xdev->header.VBNVName,64);
      //obj->vbnv = xdev->header.VBNVName;
    
	printk(KERN_INFO "XOCL: VBNV: %s", obj->vbnv);
	printk(KERN_INFO "XOCL: DDR channel count : %d\n", obj->ddr_channel_count);
	printk(KERN_INFO "XOCL: DDR channel size: %d GB\n", obj->ddr_channel_size);
...
static const struct drm_ioctl_desc xocl_ioctls[] = {
	...
	DRM_IOCTL_DEF_DRV(XOCL_EXECBUF, xocl_execbuf_ioctl,
			  DRM_AUTH|DRM_UNLOCKED|DRM_RENDER_ALLOW),
      DRM_IOCTL_DEF_DRV(XOCL_INFO_UDEV, xocl_info_udev_ioctl,
			  DRM_AUTH|DRM_UNLOCKED|DRM_RENDER_ALLOW),
};

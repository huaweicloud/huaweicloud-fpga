...
public:
    ...
    int flash( const std::string& mcs1, const std::string& mcs2, std::string flashType )
    {
        ...
        std::cout << "DSA Name: " << m_devinfo.mName << std::endl;
        std::string strDsaName( m_devinfo.mName );

        if( flashType == "" ) // if not explicitly set by command line switch, determine from DSA
        {
            bool typeFound = false;
            for( unsigned i = 0; i < flash_types.size(); i++ )
            {
                auto pair = flash_types.at( i );
                if( strDsaName.find( pair.first ) != std::string::npos )
                {
                    flashType = pair.second;
                    std::cout << "'" << pair.first << "' device found, " << pair.second << " flash type." << std::endl;
                    typeFound = true;
                    break;
                }
            }
            ...
            if( !typeFound )
            {
                ...
                return -1; // TODO: add a meaningful return code.
            }
        }
        else
        {
            std::cout << "Flash type explicityly set to '" << flashType << "'." << std::endl;
        }

        int status = 0;
        if( flashType == "spi" ) // explicit, or if unset default to SPI
        {
            if( mcs1.size() != 0 )
            {
                status = xclUpgradeFirmwareXSpi(m_handle, mcs1.c_str(), 0);
            }
            if( mcs2.size() != 0 && status == 0 )
            {
                status = xclUpgradeFirmwareXSpi(m_handle, mcs2.c_str(), 1);
            }
        }
        else if( flashType == "bpi" ) // explicit
        {
            status = xclUpgradeFirmware(m_handle, mcs1.c_str());
        }
        else // invalid type was set
        {
            std::cout << "ERROR: invalid flash type '" << flashType << "' was specified." << std::endl;
            status = -1; // TODO: some code meaning invalid option.
        }

        if( status == 0 )
        {
            std::cout << "INFO: Please cold boot your machine for flash to complete." << std::endl;
        }
        return status;
    }
    ...
    int dump(std::ostream& ostr) const {
        ...
        ostr << "MIG Calibrated: " << std::boolalpha << *(unsigned char *)&m_devinfo.mMigCalib << std::noboolalpha << "\n";
#endif
        ...
        if( ifs.good() ) {
            ...
            if (0 == iRet)
            {
                ...
                if( ifs.gcount() > 0 ) {
                    ...
                    ostr << "\nMem Topology:\n";
                    ostr << "     Bank" << "      Type" << "       Base Address" << "    Size (KB)\n";
                    numDDR = map->m_count;
                    for( unsigned i = 0; i < numDDR; i++ ) {
                        ostr << " [" << i << "] " << map->m_mem_data[ i ].m_tag;
        ...
        if( ifs.good() ) {
            struct stat sb;
            iRet = stat( iplayout_path.c_str(), &sb );
            if (0 == iRet)
            {
                char fileReadBuf[ sb.st_size ];
                ifs.read( fileReadBuf, sb.st_size );
    ...
    int dmatest(size_t blockSize) {
        ...
        if (isAREDevice) {//This is ARE device
            ...
            std::cout << "Averaging ARE hardware latency over " << (numIteration * numHops) << " hops" << std::endl;

            //printf("Latency per ARE hop for 128KB: %lu ns\n", delayPerHop);
            std::cout << "Latency per ARE hop for 128KB: " << delayPerHop << " ns" << std::endl;

            //printf("Total latency over ARE: %lu ns\n", (timeARE - timeDDR));
            std::cout << "Total latency over ARE: " << (timeARE - timeDDR) << " ns" << std::endl;
        }
        return result;
    }
   ...
   //Debug related functionality.
     ...
     *
     * Usage:
     * dd -d0 --if=in.txt --bs=4096 --count=16 --seek=0
     * dd -d0 --of=out.txt --bs=1024 --count=4 --skip=0

    ...
    public:
        DMAChannelManager(unsigned deviceIndex, unsigned count, std::ios_base::openmode mode) : mCount(count) {
            ...
            for (mIndex = 0; mIndex < static_cast<int>(mCount); ++mIndex) {
				...
				int fd = open(fileName.c_str(), (mode == std::ios_base::out) ? O_WRONLY : O_RDONLY);
				if(fd == -1)
				{
					std::cout<<"ERROR: the file "<<fileName <<" is open failed \n";
					mChannel.push_back(-1);
				}
				else
				{
					mChannel.push_back(fd);
				}
            }
            --mIndex;
        }
        ...
        bool lock() const {
            for (unsigned i = 0; i < mCount; i++) {
                ...
                for (unsigned j = 0; j < i; j++) {
                    int ret = flock(mChannel[j], LOCK_UN);
                    if(-1 == ret){
                        std::cout<<"ERROR:the channel "<<mChannel[j]<<" unlock failed, errno "<<std::endl;
                    }
                }
                return false;
            }
        ...
        void unlock() const {
            for (unsigned i = 0; i < mCount; i++) {
                int ret = flock(mChannel[i], LOCK_UN);
                if(-1 == ret){
                    std::cout<<"ERROR:the channel "<<mChannel[i]<<" unlock failed, errno "<<std::endl;
                }
            }
        }

...
#define LAPC_STATUS_0_OFFSET     0x200
#define LAPC_STATUS_1_OFFSET     0x204
#define LAPC_STATUS_2_OFFSET     0x208
#define LAPC_STATUS_3_OFFSET     0x20c
...
namespace xclxdma {
  ...
  uint64_t XDMAShim::getProtocolCheckerBaseAddress(int type) {
    switch (type) {
    case 0:
      return LAPC0_BASE;
    case 1:
  ...
  uint32_t XDMAShim::getCheckerNumberSlots(int type) {
    return getBankCount();
  }

  // Read APM performance counters
  size_t XDMAShim::xclDebugReadCheckers(xclDebugCheckersResults* aCheckerResults) {
    ...
    uint64_t statusRegisters[] = {
        ...
        LAPC_STATUS_0_OFFSET, LAPC_STATUS_1_OFFSET,
        LAPC_STATUS_2_OFFSET, LAPC_STATUS_3_OFFSET
    };
    ...
    for (int s = 0; s < numSlots; ++s) {
      ...
      std::copy(temp+XLAPC_CUMULATIVE_STATUS_0, temp+XLAPC_STATUS_0, aCheckerResults->CumulativeStatus[s]);
      std::copy(temp+XLAPC_STATUS_0, temp+XLAPC_STATUS_PER_SLOT, aCheckerResults->SnapshotStatus[s]);
    }
  ...
  // Read APM performance counters
  size_t XDMAShim::xclDebugReadCounters(xclDebugCountersResults* aCounterResults) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id()
    ...
    uint32_t temp[XAPM_D_METRIC_COUNTERS_PER_SLOT];

    for (uint32_t s=0; s < numSlots; s++) {
      for (int c=0; c < XAPM_D_METRIC_COUNTERS_PER_SLOT; c++)

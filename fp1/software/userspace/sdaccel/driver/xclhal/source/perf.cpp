...
namespace xclxdma {
  ...
  uint64_t XDMAShim::getPerfMonFifoBaseAddress(xclPerfMonType type, uint32_t fifonum) {
    if (type == XCL_PERF_MON_MEMORY) {
      ...
      if (isDSAVersion(FAST_OFFLOAD_MAJOR, FAST_OFFLOAD_MINOR, false))
        return PERFMON0_OFFSET + XPAR_AXI_PERF_MON_0_OFFSET_0;

      if (fifonum == 0) return (PERFMON0_OFFSET + XPAR_AXI_PERF_MON_0_OFFSET_0);
      if (fifonum == 1) return (PERFMON0_OFFSET + XPAR_AXI_PERF_MON_0_OFFSET_1);
      if (fifonum == 2) return (PERFMON0_OFFSET + XPAR_AXI_PERF_MON_0_OFFSET_2);
      return 0;
    }
    if (type == XCL_PERF_MON_OCL_REGION) {
      if (fifonum == 0) return (PERFMON2_OFFSET + XPAR_AXI_PERF_MON_2_OFFSET_0);
      if (fifonum == 1) return (PERFMON2_OFFSET + XPAR_AXI_PERF_MON_2_OFFSET_1);
      if (fifonum == 2) return (PERFMON2_OFFSET + XPAR_AXI_PERF_MON_2_OFFSET_2);
      return 0;
    }
    return 0;
  }
  ...
  uint64_t XDMAShim::getPerfMonFifoReadBaseAddress(xclPerfMonType type, uint32_t fifonum) {
    if (type == XCL_PERF_MON_MEMORY) {
      // Use AXI-MM to access  FIFO
      // NOTE: requires compatible change in base platform
      if (isDSAVersion(FAST_OFFLOAD_MAJOR, FAST_OFFLOAD_MINOR, false))
        return XPAR_AXI_PERF_MON_0_OFFSET_AXI_FULL;
  ...
  uint32_t XDMAShim::getPerfMonNumberFifos(xclPerfMonType type) {
    if (type == XCL_PERF_MON_MEMORY)
      return XPAR_AXI_PERF_MON_0_NUMBER_FIFO;
    if (type == XCL_PERF_MON_HOST_INTERFACE)
      return XPAR_AXI_PERF_MON_1_NUMBER_FIFO;
    if (type == XCL_PERF_MON_OCL_REGION) {
  ...
  uint32_t XDMAShim::getPerfMonNumberSamples(xclPerfMonType type) {
    if (type == XCL_PERF_MON_MEMORY) return XPAR_AXI_PERF_MON_0_NUMBER_SAMPLES;
    if (type == XCL_PERF_MON_HOST_INTERFACE) return XPAR_AXI_PERF_MON_1_NUMBER_SAMPLES;
    // TODO: get number of samples from metadata
    if (type == XCL_PERF_MON_OCL_REGION) return XPAR_AXI_PERF_MON_2_NUMBER_SAMPLES;
    return 0;
  }
  ...
  // Reset all APM  AXI stream FIFOs
  size_t XDMAShim::resetFifos(xclPerfMonType type) {
    ...
    for (int f=0; f < XPAR_AXI_PERF_MON_0_NUMBER_FIFO; f++) {
  ...
  // Start device counters performance 
  ...
  size_t XDMAShim::xclPerfMonReadCounters(xclPerfMonType type, xclCounterResults& counterResults) {
    ...
    int ret =(int)xclRead(XCL_ADDR_SPACE_DEVICE_PERFMON, intervalAddress, &sampleInterval, 4);
    if (ret < 0) return ret;
  ...
  // Clock  used in converting device  timestamps to host domain
  ...
  size_t XDMAShim::xclPerfMonStopTrace(xclPerfMonType type) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << ", "
          << type << ", Stop and reset device ..." << std::endl;
    }
  ...
  uint32_t XDMAShim::xclPerfMonGetTraceCount(xclPerfMonType type) {
    ...
    if (type == XCL_PERF_MON_MEMORY && isDSAVersion(FAST_OFFLOAD_MAJOR, FAST_OFFLOAD_MINOR, false))
      numSamples = numBytes / (XPAR_AXI_PERF_MON_0_WORD_WIDTH/8);
    else
      numSamples = numBytes >> 2;

    if (mLogStream.is_open()) {
      mLogStream << "  No. of  samples = " << std::dec << numSamples
  ...
  size_t XDMAShim::xclPerfMonReadTrace(xclPerfMonType type, xclTraceResultsVector& traceVector) {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id()
      << ", " << type << ", " << &traceVector
      << ", Reading device  stream..." << std::endl;
    }
    ...
    const uint32_t bytesPerSample = (XPAR_AXI_PERF_MON_0_WORD_WIDTH / 8);
    const uint32_t wordsPerSample = (XPAR_AXI_PERF_MON_0_WORD_WIDTH / 32);
    uint32_t numWords = numSamples * wordsPerSample;

    // Create  buffer on host (requires alignment)
    const int BUFFER_BYTES = MAX_NUMBER_SAMPLES * bytesPerSample;
    const int BUFFER_WORDS = MAX_NUMBER_SAMPLES * wordsPerSample;
#ifndef _WINDOWS
// TODO: Windows build support
//    alignas is defined in c++11
    ...
    if (type == XCL_PERF_MON_MEMORY && isDSAVersion(FAST_OFFLOAD_MAJOR, FAST_OFFLOAD_MINOR, false)) {
      ...
      // Read remainder of  not divisible by chunk size
      if (words < numWords) {
    ...
    for (uint32_t wordnum=0; wordnum < numSamples; wordnum++) {
      ...
      if (mLogStream.is_open()) {
        mLogStream << "  sample " << std::dec << wordnum << ": ";

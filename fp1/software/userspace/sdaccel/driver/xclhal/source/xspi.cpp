...
#define FLASH_BASE_ADDRESS 0x040000
 ...
 * The LSB first data format is not available in all versions of the Xilinx 
 * Device whereas the MSB first data format is supported by all the versions of
 * the Xilinx  Devices. Please check the HW specification to see if this
...
bool XDMAShim::finalTransfer(uint8_t *SendBufPtr, uint8_t *RecvBufPtr, int ByteCount)
{
  ...
  { /* Polled mode of operation */
    ...
    // poll the status register to * Transmit/Receive  data.
    while(ByteCount > 0)
    {
...
int XDMAShim::programXSpi(std::ifstream& mcsStream, const ELARecord& rd) {
  if (mLogStream.is_open()) {
  ...
  for (unsigned index = rd.mDataCount; index > 0;) {
    ...
    if (bufferIndex == WRITE_DATA_SIZE) {
      ...
      if(TEST_MODE) {
        ...
        std::cout << rd.mStartAddress << " " << rd.mStartAddress + pageIndex*PAGE_SIZE;
        std::cout << " " << address << std::endl;
      } else
      {
        if(!writePage(rd.mStartAddress + pageIndex*WRITE_DATA_SIZE))
          return -1;
        clearBuffers();
        {
...
#if 0
          if(pageIndex == 0) {
            if(!readPage(rd.mStartAddress + pageIndex*WRITE_DATA_SIZE))
              return -1;
            clearBuffers();
          }
#endif
